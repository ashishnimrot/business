...
# Usage: ./backup-and-upload-to-s3.sh [DB_HOST] [DB_PORT] [DB_USER] [DB_PASSWORD] [S3_BUCKET] [KEEP_LOCAL]
...
if [ -z "$S3_BUCKET" ]; then
    ...
    "$SCRIPT_DIR/upload-backup-to-s3.sh" "" "" "$KEEP_LOCAL"
else
    "$SCRIPT_DIR/upload-backup-to-s3.sh" "$S3_BUCKET" "" "$KEEP_LOCAL"
fi
